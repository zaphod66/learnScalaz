...
/**
  ...
  * $ sbt
  * > console
  * scala> import com.myspace.streams._
...
class TransactionProcessor(host: String, port: Int)(implicit val system: ActorSystem) extends Logging {
  ...
  def run(): Unit = {
    ...
    val summarizer = system.actorOf(Props[Summarizer], "summarizer")
    ...
    system.scheduler.schedule(0.seconds, 500.milliseconds, summarizer, LogSummaryBalance)
  }
}
